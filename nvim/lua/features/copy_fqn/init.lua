...
local function providers_by_filetype()
  local map = {}
  for _, path in ipairs(vim.api.nvim_get_runtime_file('lua/features/copy_fqn/providers/*.lua', true)) do
    local name = vim.fn.fnamemodify(path, ':t:r')
    local ok, provider = pcall(require, 'features.copy_fqn.providers.' .. name)
    if ok then
...
function M.run()
  ...
  require('features.copy_fqn.engine').run(provider)
end
